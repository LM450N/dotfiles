...
# Exportable methods.
__all__ = ['get_hive_statement', 'get_weather', 'format_to_ngsi2']
...
class LoRaProxy(webapp2.RequestHandler):
    ...
    def post(self):
        ...
        try:
            ...
            log.info('Request root: ' + str(root))
            payload = root.find(
                '{http://uri.actility.com/lora}payload_hex').text
            log.info('Received payload: ' + str(payload))
            code = payload[0:2].decode("hex")
        except Exception:
            log.warning("XML data is empty or doesn't contain payload.")
            return

        if code == "A":
            # A = Weather station for Monceau le neuf. The nearest station is
            # located at Crecy sur Serre.
            # - garden code -
            format_json_context_broker(
            ...
            # C = Monceau le neuf beehive 01
            # temperature, humidity, vibration, battery
            # format_json_context_broker(payload,"5694015311708160","crecy-sur-serre")
            format_json_context_broker(
                payload, "5650665401483264", "chalons-en-champagne")

    def get_hive_statement(self, id, payload):
        hive = {'id': id}
        hive['insideTemperature'] = int(payload[2:6], 16) / 100.00
        hive['insideHumidity'] = int(payload[6:10], 16) / 100.00
        hive['batteryLevel'] = int(payload[10:14], 16)
        return hive

    def get_weather(self, city):
        # TODO: get weather from OpenWeatherMap and return a dict.
        pass

    def format_to_ngsi2(self, hive, weather):
        # TODO: use hive statement and weather info to create a dict with
        # format NGSI2.
        pass


def format_json_context_broker(payload, hiveID, ville):
    # Temperature + humidity converted into int, need to divide by 100.0 to
    # convert into float
    # TODO: Replace by: hive = get_hive_statement(payload[0:14])
    ...
    log.warning('Float :' + str(temperature) + ',' +
                str(humidity) + ',' + str(battery))

    # Get data from openweathermap
    # TODO: Replace by: weather = get_weather(ville)
    ...
    if data_weather['cod'] == "200":
        ...
        log.warning(data_weather)

        # TODO: Replace from here to L168 by:
        # data = format_to_ngsi2(hive, weather)
        ...
        log.warning("Error openweathermap Call from %s at %s" %
                    (hiveID, str(long(time.time()))))

    return

APP = webapp2.WSGIApplication([
    ('/hostabee-lora-proxy', LoRaProxy)
], debug=DEBUG)
